...
// TODO: move all this to SpotifyWebClient
...
//        request.setValue("application/x-www-form-urlencoded ", forHTTPHeaderField: "Content-Type")
        ...
        components.queryItems = [
            ...
            URLQueryItem(name: "redirect_uri", value: SpotifyWebClient.AUTH_REDIRECT_URI),
//            URLQueryItem(name: "state", value: ),
    ...
    func handleNewLogin(oauthRedirectedURL: URL?, pkceVerifier: String) async throws {
        guard let oauthRedirectedURL = oauthRedirectedURL else {
            throw Spotify.AuthError.any(detail: "oauth failed to redirect URL")
        }
        guard let authCode = URLComponents(string: oauthRedirectedURL.absoluteString)?
            ...
            throw Spotify.AuthError.any(detail: "oauthRedirectedURL did not contain auth code")
        }
        ...
        self.loggedInUser = try JSONDecoder().decode(Spotify.CurrentUser.self, from: data)
    }
...
// note we can't make these generic since logOut is tied to an instance of SpotifyWebClient
extension SpotifyWebClient {
